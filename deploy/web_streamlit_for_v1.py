...
def prepare_generation_config():
    with st.sidebar:
        ...
        max_new_tokens = st.slider(
            'Max Generation Length', 100, 8192, 660, step=8)
        top_p = st.slider('Top P', 0.0, 1.0, 0.8, step=0.01)
        temperature = st.slider('Temperature', 0.0, 1.0, 0.7, step=0.01)
        repetition_penalty = st.slider(
            "Repetition Penalty", 1.0, 2.0, 1.07, step=0.01)
        st.button('重置聊天')
...
def combine_history(prompt):
    ...
    system = system_prompt.format(content="你是一个超级人工智能，拥有全人类的智慧汇合。"
                                  "你喜欢用文笔极佳的中文回复用户。"
                                  "你需要结合中国的文化和聊天记录中的上文话题理解并推测用户意图，按要求正确回复用户问题。"
                                  "注意使用恰当的文体和格式进行回复，尽量避免重复文字，避免重复句子，且单次回复尽可能简洁深邃。"
                                  "你深思熟虑地回复用户。"
                                  )
...
def main(model_name_or_path, adapter_name_or_path):
    ...
    model, tokenizer = load_model(
        model_name_or_path, adapter_name_or_path=adapter_name_or_path, load_in_4bit=False)
    ...
    if prompt := st.chat_input('解释一下Vue的原理'):
        ...
        with st.chat_message('robot'):
            ...
            for cur_response in generate_interactive(
                    ...
                    tokenizer=tokenizer,
                    prompt=real_prompt,
                    additional_eos_token_id=128001,  # <|end_of_text|>
